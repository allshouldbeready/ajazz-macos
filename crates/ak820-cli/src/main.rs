...
enum TftCmd {
    ...
    /// Select a firmware-provided TFT animation index
    SelectIndex {
        #[arg(long)]
        index: u8,
    },
...
fn cmd_sync_clock(json: bool) -> Result<()> {
    let datetime = ak820_protocol::commands::TftDateTime::now_local();
    let conn = ak820_protocol::Connection::open_control()?;
    conn.set_tft_datetime(datetime)?;
    if json {
        println!("{}", serde_json::json!({"ok": true, "datetime": datetime}));
    } else {
        println!(
            "TFT clock synchronised to {:04}-{:02}-{:02} {:02}:{:02}:{:02}.",
            datetime.year,
            datetime.month,
            datetime.day,
            datetime.hour,
