...
| 5b — TFT display | ✅ GIF path verified | Online firmware uses cmd 80 on `0xFF67`; supplied-driver firmware uses `0x72` plus 4096-byte reports on `0xFF68`. Its body is a 256-byte timing header, RGB565 frames, then report padding. GIFs exceeding the adjustable 1–140 frame budget sample the complete timeline; transactions close with FINISH after SAVE. | corrected spatial layout and complete GIF playback visibly confirmed on wired ANSI hardware; still-image and interrupted-upload acceptance remain separate |
| 5c — TFT presets | 🧪 beta | `tft_presets` module with 12 generated animations (2 diagnostic + 10 decorative). `apply_tft_preset` + `list_tft_presets` Tauri commands. TFT view replaces the "coming soon" placeholder. | current 4096-byte legacy stream renders complete 128×128 frames; individual presets remain pending confirmation |
| 5d — TFT image upload | 🧪 beta | `tft_image` module decodes PNG / JPEG / GIF via `image` crate, fits via Fill/Contain/Stretch, quantises to RGB565, hands to the chunked uploader. Tauri command `apply_tft_image(path, fit)`. Native file picker via `tauri-plugin-dialog`. Factory-default button via `tft_factory_default` → `SET_TFT_BUILT_IN_INDEX(0)`. | MP4 / video deferred — would more than double the binary; users decimate to GIF first |
| 5e — Live TFT presets | ⏳ pending | — | needs a polling architecture + bitmap-font rasteriser to render Battery / Volume / CPU / Clock onto the panel without uploading user pre-rendered images. Mario flagged 10 functional presets as desirable; tracked but not built yet |
