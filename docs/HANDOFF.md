...
| 5b — TFT display | 🟡 hardware pending | Online firmware uses cmd 80 on `0xFF67`; supplied-driver firmware uses `0x72` plus 4096-byte reports on `0xFF68`. Its body is a 256-byte timing header, RGB565 frames, then report padding. GIFs exceeding the frame budget sample the complete timeline; transactions close with FINISH after SAVE. | a Quadrants photo exposed and diagnosed the former 15-row offset; corrected layout awaits visual confirmation |
