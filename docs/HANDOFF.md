...
| 5b — TFT display | 🟡 hardware pending | Online firmware uses cmd 80 on `0xFF67`; supplied-driver firmware uses the recovered `0x72` transaction and 4096-byte animation stream on `0xFF68`. GIFs exceeding the frame budget are sampled across the complete timeline. Legacy transactions now close with FINISH after SAVE. | corrected 30-frame transfer completed; visible full-loop playback and menu responsiveness await user confirmation |
