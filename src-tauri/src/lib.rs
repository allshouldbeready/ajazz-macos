...
/// Serializes official-feature hardware writes, including the separate TFT
/// endpoint, so a large display transfer cannot overlap a settings commit.
#[derive(Default)]
pub struct DeviceIoGate(Mutex<()>);

#[derive(Default)]
pub struct TftUploadState {
    active: AtomicBool,
...
impl TftUploadState {
    fn begin(&self) -> Result<(), AppError> {
        self.active
            ...
            .map_err(|_| AppError::Protocol("a TFT upload is already running".into()))?;
...
async fn sync_clock(
    io: State<'_, Arc<DeviceIoGate>>,
    state: State<'_, Arc<ConnState>>,
) -> Result<TftDateTime, AppError> {
    let _gate = io.0.lock().await;
    let datetime = TftDateTime::now_local();
    state
        .with(|slot| {
            let conn = ensure_open(slot)?;
            conn.set_tft_datetime(datetime)?;
            Ok(())
        })
        .await?;
    Ok(datetime)
...
async fn upload_tft_with_progress(
    ...
    upload_state.finish();
    worker_result.map_err(|e| AppError::Protocol(format!("join upload: {e}")))?
}

#[tauri::command]
fn cancel_tft_upload(upload: State<'_, Arc<TftUploadState>>) -> bool {
    upload.cancel()
}
...
#[tauri::command]
async fn now_playing_tft_stop(state: State<'_, Arc<NowPlayingTftState>>) -> Result<(), AppError> {
    state.stop().await;
    Ok(())
}
...
pub fn run() {
    ...
    let builder = tauri::Builder::default()
        ...
        .manage(Arc::new(DeviceIoGate::default()))
        .manage(Arc::new(TftUploadState::default()))
...
mod tests {
    ...
    fn disconnect_errors_trigger_cached_handle_recovery() {
        assert!(is_stale_handle_error(&AppError::Protocol(
            "HID error: Device is disconnected".into()
        )));
        assert!(!is_stale_handle_error(&AppError::Protocol(
            "unexpected packet length".into()
        )));
    }

    #[test]
    fn tft_upload_state_rejects_overlap_and_resets_after_finish() {
        let state = TftUploadState::default();
